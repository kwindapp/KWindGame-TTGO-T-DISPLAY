...
TFT_eSprite sprite_kiter = TFT_eSprite(&tft);
...
void setup()
{
  ...
  sprite_kiter.setColorDepth(16);
  sprite_enemy1.setColorDepth(16);
  sprite_enemy2.setColorDepth(16);

  img.createSprite(TFT_HEIGHT, 100);
  sprite_kiter.createSprite(image_kiter.width, image_kiter.height);
...
void drawS(int x, int y, int frame)
{
  ...
  sprite_kiter.pushImage(0, 0, image_kiter.width, image_kiter.height, image_kiter.data[frame]);

  sprite_enemy1.pushToSprite(&img, eX[0], 56, TFT_WHITE);
  sprite_enemy2.pushToSprite(&img, eX[1], 56, TFT_WHITE);
  sprite_kiter.pushToSprite(&img, x, y-50, TFT_WHITE);
...
void checkColision()
{
  for (int i = 0; i < 2; i++)
  {
    if (eX[i] < x + image_kiter.width / 2 && eX[i] > x && y > 58)// detect coll
    {
      gameRun = 0;
